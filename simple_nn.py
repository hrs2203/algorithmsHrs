...
class NeuralNetwork:
    ...
    def backward(
        ...
        for layer in range(1,self.num_layers+1):
            l2_loss += np.sum(np.square(self.params[f"W{layer}"]))
        l2_loss = (reg/self.output_size)*l2_loss
        ...
        # ================== l2 correction =======================
        self.gradients[f"dw{self.num_layers}"] += (reg/self.output_size)*np.sum(
            np.square(self.params[f"W{self.num_layers}"])
        )
        # ========================================================
        ...
        for layer in range(self.num_layers-1, 0, -1):
            ...
            # ================== l2 correction =======================
            self.gradients[f"dw{layer}"] += (reg/self.output_size)*np.sum(
                np.square(self.params[f"W{layer}"])
            )
            # ========================================================
    ...
    def backward_adam(
        ...
        for layer in range(1, self.num_layer+1):
            self.first_momentum[f"w{layer}"] = 0.0
            self.second_momentum[f"w{layer}"] = 0.0
        # ==================================================
        ...
        # ================== Adam Operation ====================
        self.first_momentum[f"w{self.num_layers}"] = ( 
            (beta1 * self.first_momentum[f"w{self.num_layers}"]) + ((1-beta1)*dl)
        )
        self.second_momentum[f"w{self.num_layers}"] = ( 
            (beta2 * self.second_momentum[f"w{self.num_layers}"]) + ((1-beta2)*dl*dl)
        )
        self.gradients[f"dw{self.num_layers}"] = (
            self.first_momentum[f"w{self.num_layers}"] / (
                np.sqrt(self.second_momentum[f"w{self.num_layers}"]) + 1e-7
            )
        )
        # ======================================================
        ...
        for layer in range(self.num_layers-1, 0, -1):
            ...
            # ================== Adam Operation ====================
            self.first_momentum[f"w{layer}"] = ( 
                (beta1 * self.first_momentum[f"w{layer}"]) + ((1-beta1)*dl)
            )
            self.second_momentum[f"w{layer}"] = ( 
                (beta2 * self.second_momentum[f"w{layer}"]) + ((1-beta2)*dl*dl)
            )
            self.gradients[f"dw{layer}"] = (
                self.first_momentum[f"w{layer}"] / (
                    np.sqrt(self.second_momentum[f"w{layer}"]) + 1e-7
                )
            )
            # ======================================================
